...
def run_isomut_on_block(chrom,from_pos,to_pos,
    ...
    cmd+=' '.join(map(str,[min_sample_freq,min_other_freq,cov_limit,
...
def run_isomut_in_parallel(input_dir,bam_filenames,output_dir,ref_fasta, min_block_no=100,n_conc_blocks=2):
    #define blocks and create args
    blocks=define_parallel_blocks(ref_genome=ref_fasta,min_block_no=min_block_no)
    args=[ (block[0],block[1],block[2],input_dir,bam_filenames,output_dir,ref_fasta) for block in blocks]
    ...
    while (len(procs) != len(args)):
        for i in xrange(1,n_conc_blocks+1):
